...
class PlayersCommand(BaseCommand):
    ...
    def description(self) -> str:
        return "마인크래프트 서버에 접속한 플레이어 수와 목록을 조회합니다"
    ...
    async def execute(self, interaction: discord.Interaction):
        ...
        try:
            # 플레이어 정보 조회 (비동기, mcrcon-nsg 사용)
            player_count, players = await rcon.get_players()
            
            # 임베드 생성
            embed = discord.Embed(
                title="🎮 마인크래프트 서버 플레이어 정보",
            ...
            if player_count == 0:
                ...
                embed.description = f"현재 **{player_count}명**의 플레이어가 접속 중입니다."
                embed.add_field(
                    name="접속 인원",
